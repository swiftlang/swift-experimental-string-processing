// TODO: This might be interesting in the future, but for now
// we make trees

//public protocol ASTAction {
//  // TODO: associated types, or just make them produce AST nodes?
//
//  // MARK: Group
//  static func capture(_ a: AST, _ sr: SourceLocation?) -> AST
//
//  static func nonCapture(_ a: AST, _ sr: SourceLocation?) -> AST
//
//  // MARK: Quantification
//
//  static func zeroOrMore(
//    _ kind: Quantifier.Kind, _ a: AST, _ r: SourceLocation?
//  ) -> AST
//  static func oneOrMore(
//    _ kind: Quantifier.Kind, _ a: AST, _ r: SourceLocation?
//  ) -> AST
//  static func zeroOrOne(
//    _ kind: Quantifier.Kind, _ a: AST, _ r: SourceLocation?
//  ) -> AST
//}
