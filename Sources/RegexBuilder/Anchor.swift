...
extension Anchor: RegexComponent {
  var baseAssertion: _DSLTree._AST._AssertionKind {
    switch kind {
    case .startOfSubject: return ._startOfSubject(isInverted)
    case .endOfSubjectBeforeNewline: return ._endOfSubjectBeforeNewline(isInverted)
    case .endOfSubject: return ._endOfSubject(isInverted)
    case .firstMatchingPositionInSubject: return ._firstMatchingPositionInSubject(isInverted)
    case .textSegmentBoundary: return ._textSegmentBoundary(isInverted)
    case .startOfLine: return ._startOfLine(isInverted)
    case .endOfLine: return ._endOfLine(isInverted)
    case .wordBoundary: return ._wordBoundary(isInverted)
    }
  }
  
  public var regex: Regex<Substring> {
    Regex(_node: .atom(.assertion(baseAssertion)))
  }
}
...
public struct Lookahead<Output>: _BuiltinRegexComponent {
  ...
  public init<R: RegexComponent>(
    ...
    self.init(_node: .nonCapturingGroup(._lookahead, component.regex._root))
  }
...
public struct NegativeLookahead<Output>: _BuiltinRegexComponent {
  ...
  public init<R: RegexComponent>(
    ...
    self.init(_node: .nonCapturingGroup(._negativeLookahead, component.regex._root))
  }
