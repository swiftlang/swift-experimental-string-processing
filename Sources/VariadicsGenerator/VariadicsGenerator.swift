...
struct VariadicsGenerator: ParsableCommand {
    ...
    output("""
      ...
      extension \(kind.rawValue) {
        ...
        public init<\(params.genericParams)>(
          ...
          self.init(_node: .quantification(.\(kind.astQuantifierAmount), kind, component.regex._root))
        }
      }
      ...
      \(kind == .zeroOrOne ?
        ...
        extension \(concatBuilderName) {
          ...
          public static func buildLimitedAvailability<\(params.genericParams)>(
            ...
            .init(_node: .quantification(.\(kind.astQuantifierAmount), .default, component.regex._root))
          }
        }
        """ : "")
  ...
  func emitAtomicGroup(arity: Int) {
    ...
    func node(builder: Bool) -> String {
      """
      .nonCapturingGroup(._atomicNonCapturing, component\(
        builder ? "()" : ""
      ).regex._root)
      """
    }
    ...
    output("""
      ...
      extension \(groupName) {
        ...
        public init<\(genericParams)>(
          ...
          self.init(_node: \(node(builder: false)))
        }
      }
      ...
      extension \(groupName) {
        ...
        public init<\(genericParams)>(
          ...
          self.init(_node: \(node(builder: true)))
        }
      }
